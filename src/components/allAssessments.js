...
import { Assessments } from "./assessments"; // Assessment Component from assessment.js
...
export class AllAssessments extends React.Component { //Accessing the React Functionality & marked for export
    ...
    componentDidMount(){ // When component becomes active in the view it will tell it what to do 
        ...
        .then((response)=>{
            this.setState({assessments:response.data}) // My JSON myassessments
    ...
    state = {// Holding the components data
        assessments: [ ]  // An array of assessments
    }
    ...
    render() {
        return ( // Where the visual content of this component will be added
            <div>
                ...
                <Assessments assessments={this.state.assessments} ReloadData={this.ReloadData}></Assessments> {/* Imported from the assessments Component & state  is used to contain data or information about the component*/}
            </div>
        );
    }
}